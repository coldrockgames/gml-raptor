...
function StateMachine(_owner) : BindableDataBuilder() constructor {
	...
	static add_state = function(_name, _on_enter = undefined, _on_step = undefined, _on_leave = undefined) {
		...
		if (existing == undefined) {
			...
			st.data = data;
			array_push(__states, st);
			if (string_starts_with(_name, "bc:")) {
				var bctitle		= string_skip_start(_name, 3);
				if (string_trim(bctitle) != "") {
					st.__bc_name	= bctitle;
					st.__bc_rec		= $"STM_{name_of(self)}_{bctitle}";
					BROADCASTER.add_receiver(self, st.__bc_rec, bctitle, 
						function(bc) {
							var stname = string_concat("bc:", bc.title);
							var stinst = get_state(stname);
							if (stinst != undefined) {
								stinst.data.bc = bc;
								set_state(stname);
							}
	...
	static delete_state = function(name) {
		if (get_active_state_name() == name) 
			return;
		var delidx		= -1;
		var foundstate	= undefined;
		
		for (var i = 0, len = array_length(__states); i < len; i++) {
			if (__states[@i].name == name) {
				foundstate = __states[@i];
				delidx = i;
				break;
			}
		}
		if (delidx != -1) {
			if (foundstate.__bc_rec != undefined) {
				BROADCASTER.remove_receiver(foundstate.__bc_rec);
				foundstate.__bc_rec = undefined;
			}
